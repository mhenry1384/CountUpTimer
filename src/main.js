...
function parseConfig(json) {
  ...
  if (json.date && json.time) {
    return /* @__PURE__ */ new Date(`${json.date}T${json.time}`);
  }
  if (json.date) {
...
function renderCountdown(components) {
  ...
  if (container.dataset.kinds !== kindKey) {
    ...
    rows.forEach(
      (row) => container.appendChild(createBar(row.kind, row.value, row.label, row.max))
    );
  } else {
    rows.forEach((row, i) => {
...
function toTauriPosition(position) {
...
async function applyWindowPosition(position) {
